...
function renderCards(container) {
  ...
  grid.innerHTML = AREAS.map(area => {
    ...
    return `<div class="kpi-card" style="border-top:3px solid ${tempColor}">
      <div class="kpi-label" style="font-size:10px">${escapeHtml(area)}</div>
      <div class="kpi-value" style="color:${tempColor};font-size:24px">${r ? escapeHtml(r.temperatura) + '°C' : '—'}</div>
      <div style="font-size:11px;color:var(--muted);margin-top:4px">${r ? 'Ultima: ' + escapeHtml(r.hora.slice(0, 5)) : 'Sin datos hoy'}</div>
...
function renderTable(container) {
  ...
  tbody.innerHTML = tempData.map(r => `<tr>
    <td style="font-family:monospace;font-size:13px">${escapeHtml(r.hora.slice(0, 5))}</td>
    ...
    <td style="font-size:11px;color:var(--muted)">${escapeHtml(r.observaciones || '—')}</td>
  </tr>`).join('');
}
